      ...
      )
{
   ...
   /* Note: playlist variable is no longer used - we accumulate results instead */
   (void)playlist;
   (void)db_playlist_path;
   ...
   {
      RARCH_LOG("[Scanner] Matched \"%s\" to \"%s\"\n", entry_lbl, db_playlist_base_str);
      if (retroarch_override_setting_is_set(RARCH_OVERRIDE_SETTING_DATABASE_SCAN, NULL))
