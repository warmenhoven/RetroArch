...
static void frame_consumer(void *arg)
{
   ...
   for (token = 0; token < 100000; token++)
   {
      ...
      if (retro_spsc_read(&s->q, frame, s->width) != s->width) s->mismatches++;
      for (i = 0; i < s->width; i++)
         if (frame[i] != (uint8_t)(token * 13 + i)) s->mismatches++;
   }
}

static int run_frame_stress(void)
{
   frame_stress_t s;
   sthread_t *consumer;
   uint8_t frame[44];
   ...
   for (s.width = 22; s.width <= 44; s.width *= 2)
   {
      ...
      retro_spsc_free(&s.q);
      if (s.mismatches) return 1;
   }
   puts("[pass] frame stress: 100000 frames each of 22/44 bytes, 0 mismatches");
   return 0;
}
...
int main(void)
{
   if (run_frame_checks() != 0 || run_frame_stress() != 0) return 1;
