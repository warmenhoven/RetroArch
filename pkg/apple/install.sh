...
# Try to find the device using xctrace
UDID=$(echo "$RAW_OUTPUT" | grep -i "$DEVNAME" | grep -v Simulator | sed -n 's/.*(\([^)]*\))$/\1/p' | head -1)
...
if [ -z "$UDID" ]; then
    ...
    # Try to get UDID from devicectl (it's in a different column)
    DEVICE_LINE=$(echo "$DEVICECTL_OUTPUT" | grep -i "$DEVNAME" | grep -v Simulator | head -1)
    if [ -n "$DEVICE_LINE" ]; then
        # Extract UDID by matching the hex pattern (00000000-XXXXXXXXXXXXXXXX format)
        UDID=$(echo "$DEVICE_LINE" | grep -oE '[0-9A-Fa-f]{8}-[0-9A-Fa-f]{16}')
...
if [ -n "$UDID" ]; then
    ...
    if ! $XCRUN xcodebuild -showdestinations -workspace "$WORKSPACE" -scheme "$SCHEME" 2>&1 | grep -q "$UDID"; then
        ...
        # Try to find iPhone in xcodebuild destinations
        XCODE_UDID=$($XCRUN xcodebuild -showdestinations -workspace "$WORKSPACE" -scheme "$SCHEME" 2>&1 | grep -E "platform:iOS.*name:iPhone" | grep -v Simulator | sed -n 's/.*id:\([^,]*\).*/\1/p' | head -1)
        
        if [ -n "$XCODE_UDID" ]; then
            echo "Found iPhone UDID from xcodebuild destinations: $XCODE_UDID"
            UDID="$XCODE_UDID"
        else
            echo "Could not find iPhone in xcodebuild destinations"
...
DEVICE_STATUS=$($XCRUN devicectl list devices 2>/dev/null | grep "$DEVNAME" | grep -v Simulator | head -1)
